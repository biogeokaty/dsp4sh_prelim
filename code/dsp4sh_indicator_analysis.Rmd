...
meta_df_alt <- read.csv(here("data_processed", "05_meta_df_alt.csv"))
...
```{r make tables needed for map, echo=FALSE}
...
```{r Figure 1 - map of DSP4SH sites, echo=FALSE}
...
ggsave(here("figs", "ms_figs", "fig1_project_map.png"), height=5, width=8, units="in", dpi=400)
```

Generate overview table of project information:

```{r Table 1 - dsp4sh project overview table, echo=FALSE}
...
write_csv(project_table, here("figs", "ms_figs", "table1_project_table.csv"))
```

Table with more detail on each project treatment:

```{r project and treatment details table, echo=FALSE}
...
```{r range of environmental characteristics across all sites, echo=FALSE}
...
```{r range in indicator values across all sites, echo=FALSE}
...
```{r range in indicator values in reference sites only, echo=FALSE}
...
```{r number of studies in each climate, include=FALSE}
project %>%
  distinct(project, climate)
```

```{r range in surface soil clay content and bulk density, include=FALSE}
...
```{r indicator boxplots, echo=FALSE, message=FALSE}
...
plot_list <- purrr::map(.x = indicators,
                 .f = ~{
                   surf_long %>% 
                     filter(indicator == .x) %>%
...
```{r indicator boxplots faceted by project, echo=FALSE, message=FALSE}
projects <- surf_long %>%
  distinct(project) %>%
  pull(project) %>%
  as.character()
...
```{r Figure S7 - SOC Boxplots, include=FALSE}
# Pull out just SOC boxplot
soc_boxplot <- ggplot(surf_long %>% filter(indicator=="soc_pct"),
                     ...
                     theme(axis.text.x = element_text(angle = 45, hjust=1))
soc_boxplot
ggsave(here("figs", "ms_figs", "figsupp7_soc_boxplot.png"), width=8, height=5.5, units="in", dpi=400)
```
...
```{r Figure 2 - indicator response to management across precipitation gradient, echo=FALSE, message=FALSE}
...
ind_full
ggsave(here("figs", "ms_figs","fig2_indicator_patt_precip.png"), width=11, height=9, units="in", dpi=400)
```

```{r alternate version of precipitation gradient figure for SSSA poster}
surf_avg_clim_sssa <- surf_avg_clim %>%
  filter(soil!="Pullman", soil!="Canton", soil!="Amarillo", soil!="Drummer") %>%
  mutate(label=factor(label, levels=c("BAU", "SHM", "Ref")))

map_plot <- ggplot(surf_avg_clim_sssa, aes(x=fct_reorder(soil, map_site), y=map_site)) +
  geom_point(color="dodgerblue4") +
  geom_line(aes(group=1), color="dodgerblue2") +
  labs(y="MAP (mm)") +
  theme_katy_grid() +
...
agg_box <- ggplot(surf_avg_clim_sssa, aes(x=fct_reorder(soil, map_site), y=kssl_wsa, 
                                     fill=label)) +
  geom_boxplot_pattern(lwd=0.7, pattern="stripe", pattern_alpha=0.7, pattern_density = 0.01,
                       aes(pattern_fill = label,
                           pattern_angle = label,
                           pattern_spacing = label,
                           pattern_color = label)) +
  labs(x="Soil Series", y="Aggregate Stability (%)") +
  scale_fill_manual(values=c("#FED789FF","#72874EFF","#476F84FF"),
                    name="Management") +
  scale_pattern_fill_manual(values=c("#FED789FF","#72874EFF","#476F84FF"),
                    name="Management") +
  scale_pattern_angle_manual(name="Management",
                             values=c(0,135,45)) +
  scale_pattern_spacing_manual(name="Management",
                             values=c(1,0.04,0.04)) +
  scale_pattern_color_manual(name="Management",
                             values=c("#FED789FF", "gray20", "gray20")) +
  theme_katy_grid() +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
agg_box

alt_leg <- get_legend(agg_box + theme(legend.box.margin = margin(0, 0, 0, 12)))


alt_grid <- plot_grid(map_plot, agg_box,
                       ncol=1, rel_heights=c(.6, 1), align="v")
alt_grid

ggsave(here("figs", "sssa","indicator_patt_precip.png"), width=7, height=5.5, units="in", dpi=400)
```
...
```{r mean indicator values in all systems, echo=FALSE, message=FALSE}
...
```{r Table 2 - mean indicator values in reference only, echo=FALSE}
indicator_summary_ref <- indicator_summary_wide %>%
  filter(label=="Ref")
flextable(indicator_summary_ref)
write_csv(indicator_summary_ref, here("figs", "ms_figs", "table2_indicator_summary_ref.csv"))
```

Table of mean indicator values by land use:

```{r mean indicator values by land use, echo=FALSE, message=FALSE}
...
```{r range in mean indicator values across projects, echo=FALSE, message=FALSE}
...
```{r cv for overall dataset grouped by indicator, echo=FALSE}
...
Correlation matrices (full version and reduced version with fewer variables):

```{r Figure S1 - correlation matrix of soil health indicators, echo=FALSE}
...
ggsave(here("figs", "ms_figs", "figsupp1_surface_indicator_corrplot.png"), height=9, width=9, units="in", dpi=400)
...
```{r Figure 3 - PCA grid, include=FALSE}
# Put soil and management groupings together in a grid
plot_grid(pca_soil, pca_mgmt, ncol=1)
ggsave(here("figs", "ms_figs","fig3_indicator_pca_grid.png"), width=7, height=10, units="in", dpi=400)
```
...
```{r alternate PCA figure for SSSA poster}
pca_soil_sssa <- autoplot(imp_pca, data=surf, colour="soil", shape="soil", size=3,
                     loadings.label = TRUE, loadings.label.size = 4,
                     loadings.label.colour = 'black', loadings.color="black", 
                     loadings.label.repel=TRUE) +
  scale_color_paletteer_d("palettetown::deoxys", name="Soil Series") +
  scale_shape_manual(values=c(15,16,17,18,19,20,3,4,7,8,10,12,6,13),
                     name="Soil Series") +
  geom_hline(yintercept=0, linetype="dashed") +
  geom_vline(xintercept=0, linetype="dashed") +
  theme_katy()
...
pca_mgmt_sssa <- autoplot(imp_pca, data=surf, color="label", shape="label", size=3,
                     loadings.label = TRUE, loadings.label.size = 4,
                     loadings.label.colour = 'black', loadings.color="black", 
                     loadings.label.repel=TRUE) +
  scale_color_manual(values=c("#FED789FF","#72874EFF","#476F84FF"),
...
plot_grid(pca_soil_sssa, pca_mgmt_sssa, ncol=2)

ggsave(here("figs", "sssa","pca.png"), width=11, height=5, units="in", dpi=400)
```

## Identify environmental covariates of indicators using glmulti and partial regressions

We can try to tease out some of the drivers of variability in indicator values across the dataset - this is helpful to understand if differences in indicator values are driven by soil, climate, land use, etc. Here, glmulti is used to identify the best-fit model for each indicator with environmental variables as predictors.

```{r glmulti to select best model predicting values of each indicator, include=FALSE}
...
multi <- ind_long %>%
  group_by(indicator) %>%
  nest() %>%
  mutate(multi_obj = purrr::map(data, ~glmulti("value", c("mat", "map", "lu", "till", "clay_tot_psa"), data=.x,
  ...
  mutate(part_r2 = purrr::map(lm_obj, ~as.data.frame(sensemakr::partial_r2(.x)) 
                       %>% rownames_to_column() %>% rename(var = "rowname", partial_r2 = "sensemakr::partial_r2(.x)"))) %>%
  mutate(lm_tidy = purrr::map(lm_obj, broom::glance)) %>%
  mutate(lm_coefs = purrr::map(lm_obj, broom::tidy)) 
```

Make top model for each indicator into a nice table:

```{r table with top models for each indicator, echo=FALSE}
multi_table <- multi %>%
  ungroup() %>%
...
Pull out the partial R^2^ values for all models:

```{r table of partial R2 values for all models, echo=FALSE}
...
Plot the partial R^2^ for significant predictors of each indicator:

```{r plot partial r2 for significant predictors of each indicator, echo=FALSE}
# filter data to plot
...
-   SOC% is the most strongly predicted by environmental and land use covariates
-   Adjusted R^2^ for the best model was 0.77, and the predictors were MAT, land use, and soil clay %
    -   The strongest individual covariates were MAT (partial R^2^=0.41) and rangeland land use (partial R^2^=0.34)
-   B-glucosidase was most strongly influenced by MAP (partial R^2^=0.65)
-   KSSL aggregate stability was the indicator most strongly influenced by tillage (partial R^2^=0.43)
-   Acid phosphatase strongly influenced by rangeland land use
-   Indicators the least well-predicted by covariates (best models had adjusted R^2^\<0.5): bulk density, alkaline phosphatase, respiration, Yoder aggregate stability, phosphodiesterase, bglucosaminidase (NAG)
...
```{r glmulti to select best model - reference sites, include=FALSE}
multi_ref <- ind_long %>%
  filter(label=="Ref") %>%
  group_by(indicator) %>%
  nest() %>%
  mutate(multi_obj = purrr::map(data, ~glmulti("value", c("mat", "map", "lu", "clay_tot_psa"), data=.x,
                                      level = 1, maxsize = 3, confsetsize = 128))) %>%
  mutate(weight = purrr::map(multi_obj, ~(weightable(.x) %>% slice_head(n=1)))) %>%
  transmute(indicator, data, weight) %>%
  unnest(cols = c(weight)) %>%
...
```{r table with top models for each indicator - ref sites, echo=FALSE}
multi_table_ref <- multi_ref %>%
  ...
  select(indicator, model, adj.r.squared, p.value, AIC, nobs, sig) %>%
  mutate(across(where(is.numeric), ~ round(.x, 3)),
         indicator = str_replace_all(indicator, indicator_labs))
flextable(multi_table_ref)
write_csv(multi_table_ref, here("figs", "ms_figs","table3_indicator_lm_ref.csv"))
```

Pull out the partial R^2^ values for all models:

```{r table of partial R2 values for all models - ref sites, echo=FALSE}
...
Plot partial R^2^ of significant predictors in reference sites:

```{r Figure 4 - plot partial r2 of significant predictors - reference sites, echo=FALSE}
...
ggplot(multi_partials_2plot_ref, aes(fill=var, 
  ...
  theme_katy() +
  coord_flip()
ggsave(here("figs", "ms_figs","fig4_indicator_partial_r2_ref.png"), height=5, width=9, units="in", dpi=400)
```

Takeaways:

-   Arylsulfatase was the best predicted by environmental covariates (adjusted R^2^=0.955), followed by SOC (adjusted R^2^=0.936)

    -   top model for both included MAP, land use, and soil clay content

-   Significant models with R^2^\>0.5 determined for all indicators except for KSSL WSA
...
```{r alt version of figure 4 for sssa, include=FALSE}
ggplot(multi_partials_2plot_ref, aes(fill=var, 
                    y=partial_r2, x=fct_relevel(indicator, rev(indicator_plotting_order)))) + 
  geom_bar_pattern(position="stack", stat="identity", 
                   pattern="stripe", pattern_alpha=0.7, pattern_density = 0.01,
                   aes(pattern_fill = var,
                           pattern_angle = var,
                           pattern_spacing = var,
                           pattern_color = var)) +
  geom_label(aes(label = partial_r2), size=2.5, fontface="bold", label.size = NA, 
  ...
  scale_fill_manual(name="Predictor variable", 
                    ...
                    labels=c("LU - Rangeland", "LU - Grass", "LU - Forest",
                             "Soil Clay %", "MAP", "MAT"),
                    guide = guide_legend(reverse = TRUE)) +
  scale_pattern_fill_manual(name="Predictor variable", 
                    values=var_fill_ref,
                    labels=c("LU - Rangeland", "LU - Grass", "LU - Forest",
                             "Soil Clay %", "MAP", "MAT"),
                    guide = guide_legend(reverse = TRUE)) +
  scale_pattern_angle_manual(name="Predictor variable", 
                               values=c(134, 135, 0, 45, 45, 0),
                    labels=c("LU - Rangeland", "LU - Grass", "LU - Forest",
                             "Soil Clay %", "MAP", "MAT"),
                    guide = guide_legend(reverse = TRUE)) +
  scale_pattern_spacing_manual(name="Predictor variable", 
                               values=c(0.04, 0.02, 0.04, 0.02, 0.04, 1),
                    labels=c("LU - Rangeland", "LU - Grass", "LU - Forest",
                             "Soil Clay %", "MAP", "MAT"),
                    guide = guide_legend(reverse = TRUE)) +
  scale_pattern_color_manual(name="Predictor variable", 
                               values=c("gray20", "gray20", "gray20", "gray20", "gray20", "#35978f"),
                    labels=c("LU - Rangeland", "LU - Grass", "LU - Forest",
                             "Soil Clay %", "MAP", "MAT"),
                    guide = guide_legend(reverse = TRUE)) +
  theme_katy() +
  theme(axis.text.x=element_text(angle=45, hjust=1))
ggsave(here("figs", "sssa","indicator_partial_r2_ref.png"), height=5.5, width=7, units="in", dpi=400)

# Get just the table
multi_partials_ref_table_sssa <- multi_partials_2plot_ref %>%
  select(indicator, adj.r.squared, var, partial_r2) %>%
  pivot_wider(names_from = var,
              values_from = partial_r2) %>%
  arrange(desc(adj.r.squared))
write_csv(multi_partials_ref_table_sssa, here("figs", "sssa", "partial_regression_table.csv"))
...
```{r prepare data for effect size calculation, results='hide'}
# Pivot data longer
meta_long <- meta_df_alt %>%
...
```{r calculate effect sizes, results='hide'}
...
```{r calculate random effects models for all projects with no moderator variables, results='hide'}
...
```{r forest plots and table of significant indicators, echo=FALSE, message=FALSE}
...
plot_list3 <- map2(.x = indicators2,
                  ...
                  .f = ~{
                    ...
                    ggsave(here("figs", "indicator_es_alt",glue::glue("es_", .x, "_", .y, ".png")), 
...
-   ACE is positive in almost every Ref case, more variable in SHM but still trends positive

-   Many enzymes tend to be positive in most Ref cases, but response is inconsistent in SHM conditions

-   Bulk density is negative in Ref cases (meaning less compaction, which makes sense), and is not different between BAU and SHM cases

-   KSSL WSA are almost always positive

-   Both POX-C and SOC are fairly positive in Ref cases, but unchanged in SHM

-   Soil respiration is all over the place

-   Yoder MWD tends to be positive in Ref cases, but inconsistent in SHM

```{r Figure S6 - SOC forest plots, include=FALSE}
soc_ref_forest_plot <- ggplot(es_asp_rma_plot_df %>% filter(indicator=="soc_pct", label=="Ref"),
                              aes(x=factor(study, levels=rev(study)),y=es,
                                 ymax=es+se,ymin=es-se,size=factor(type),colour=factor(type))) +
                      geom_pointrange() +
                      coord_flip() + 
                      geom_hline(yintercept=0, lty=2,linewidth=1) +
...
soc_shm_forest_plot <- ggplot(es_asp_rma_plot_df %>% filter(indicator=="soc_pct", label=="SHM"),
                              aes(x=factor(study, levels=rev(study)),y=es,
                                 ymax=es+se,ymin=es-se,size=factor(type),colour=factor(type))) +
                      geom_pointrange() +
                      coord_flip() + 
...
soc_forest_plot_grid <- plot_grid(soc_ref_forest_plot, soc_shm_forest_plot,
                                  ncol=1, labels=c("A", "B"), align="v")
soc_forest_plot_grid
ggsave(here("figs", "ms_figs","figsupp6_soc_forest_plot_grid.png"), width=7, height=10, units="in", dpi=400)
```

```{r Figure S6 - WSA vs SOC forest plots, include=FALSE}

# Filter dataframe with only SOC and aggregate stability, remove studies that don't have both
soc_wsa <- es_asp_rma_plot_df %>%
  filter(indicator=="soc_pct" | indicator=="kssl_wsa"
         ) %>%
  filter(type!="summary"
         ) %>%
  filter(study!="Illinois_Ref", study!="Illinois_NT", study!="TexasA&MPt-1_Rangland", 
         study!="TexasA&MPt-1_No Till - Mixed", study!="TexasA&MPt-1_No Till - Rye")

# Make a vector for nicer study name printing
soc_wsa_study_labs_ref <- c(
...
# Make plot for Ref condition
soc_wsa_ref_forest_plot <- ggplot(soc_wsa %>% filter(label=="Ref"),
                                  aes(x=study,
                                  y=es,ymax=es+se,ymin=es-se,
                                  colour=indicator)) +
  geom_pointrange() +
  coord_flip() + 
  geom_hline(yintercept=0, lty=2,linewidth=1) +
  labs(x="Study", y="Log response ratio") + 
  scale_x_discrete(labels=soc_wsa_study_labs_ref, limits=rev) +
  scale_color_manual(values=c("#3E356BFF", "#49C1ADFF"),
                     name="Indicator",
                     labels=c("Aggregate stability", "SOC %")
                     ) +
  theme_katy_grid()

# Make study labels for SHM condition
soc_wsa_study_labs_shm <- c(
...
# Make plot for SHM condition
soc_wsa_shm_forest_plot <- ggplot(soc_wsa %>% filter(label=="SHM"),
                                  aes(x=study,
                                  y=es,ymax=es+se,ymin=es-se,
                                  colour=indicator)) +
  geom_pointrange() +
  ...
  geom_hline(yintercept=0, lty=2,linewidth=1) +
  labs(x="Study", y="Log response ratio") + 
  scale_x_discrete(labels=soc_wsa_study_labs_shm, limits=rev) +
  scale_color_manual(values=c("#3E356BFF", "#49C1ADFF"),
                     name="Indicator",
                     labels=c("Aggregate stability", "SOC %")
                     ) +
...
# Put figures together in grid
soc_wsa_forest_plot_grid <- plot_grid(soc_wsa_ref_forest_plot + theme(legend.position="none", axis.title.x = element_blank()), 
                                  soc_wsa_shm_forest_plot + theme(legend.position="none"),
                                  ncol=1, labels=c("A", "B"), align="v")

# Extract legend
soc_wsa_leg <- get_legend(soc_wsa_ref_forest_plot)

# Add legend to figure
soc_wsa_forest_plot_grid_leg <- plot_grid(soc_wsa_forest_plot_grid, soc_wsa_leg, rel_widths = c(3, .7))

# Show figure and save
soc_wsa_forest_plot_grid_leg
ggsave(here("figs", "ms_figs","figsupp6_soc_wsa_forest_plot_grid.png"), width=9, height=7.5, units="in", dpi=400)
```

It's also helpful to plot the overall effect size for each indicator:

```{r Figure 5 - plot of summary effect sizes for all indicators, echo=FALSE}
...
ggplot(es_rma_summary, aes(x=fct_relevel(indicator, rev(indicator_plotting_order)), y=es, ymax=es+se, ymin=es-se, color=label)) + 
  ...
  theme_katy()
ggsave(here("figs", "ms_figs", "fig5_indicator_effect_sizes.png"), width=8, height=5.5, units="in", dpi=400)
```

```{r alternate version of figure 5 for SSSA, include=FALSE}
# make dataframe for plotting
ggplot(es_rma_summary, aes(x=fct_relevel(indicator, rev(indicator_plotting_order)), y=es, ymax=es+se, ymin=es-se, 
                           color=label, shape=label)) + 
  geom_pointrange(size=1, linewidth=1) +
  geom_text(aes(label=ifelse(sig=="significant", "*", "")), 
            color="black", position=position_nudge(x=0.3), size=5) +
  coord_flip() + 
  geom_hline(yintercept=0, lty=2,linewidth=1) +
  labs(x="Indicator", y="Log response ratio") +
  scale_x_discrete(labels=indicator_labs) +
  scale_color_manual(values=c("#72874EFF","#476F84FF"),
                    breaks=c("SHM", "Ref"),
                    name="Management") +
  scale_shape_discrete(breaks=c("SHM", "Ref"),
                    name="Management") +
  theme_katy()
ggsave(here("figs", "sssa", "indicator_effect_sizes.png"), width=8, height=5.5, units="in", dpi=400)
```

Make table with n for interpretability:

```{r table with observation n for random-effects models, echo=FALSE}
...
The indicators with the largest effect sizes are: arylsulfatase, aggregate stability (KSSL), aggregate stability (Yoder MWD), ACE, and SOC%

-   Biggest difference in results from combining similar treatments is that effect sizes for aggregate stability, SOC%, POX-C, and BG activity in SHM treatment are no longer significantly > 0 

-   Indicators with the biggest effect sizes in the Ref state (arylsulfatase and both aggregate stabilities) are much lower in the SHM state. Aggregate stability (both methods) is not significantly higher in SHM vs BAU.

-   ACE and AS are only two indicators with effect sizes significantly >0 in SHM 

```{r Table 4 - table summarizing overall random effects model, echo=FALSE}
...
write_csv(rma_report_table, here("figs", "ms_figs","table4_rma_report_table.csv"))
```
...
```{r random effects model with mat as a moderator variable, echo=FALSE}
...
```{r Figure S2 - plot mat as a moderator variable, echo=FALSE}
# MAT - Significant for SOC% and arysulfatase

# Make vector of indicators 
mat_indicators <- c("arylsulfatase", "soc_pct")
                 ...
                 .f = ~{
                   es_asp %>% 
                     ...
                     stat_cor(aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~")),
                              size=4) +
                     scale_colour_manual(values=c("#476F84FF", "#72874EFF"), name="Management") +
...
mat_aryl <- pluck(mat_plots, 1)
mat_soc <- pluck(mat_plots, 2)

mat_grid <- plot_grid(mat_aryl + theme(legend.position="none", axis.title.x=element_blank()),
                      mat_soc + theme(legend.position="none", axis.title.x=element_blank(), axis.title.y=element_blank()),
                      align = 'vh',
                      labels = c("A", "B"),
                      hjust = -1,
                      nrow = 1)
...
mat_title <- ggdraw() +
  draw_label(expression("Mean annual temperature"~(degree*C)), color="black", vjust=0, y=0.05)

mat_grid_title <- plot_grid(mat_grid, mat_title,
                            ncol=1, rel_heights = c(1,0.1))

mat_leg <- get_legend(mat_aryl)

plot_grid(mat_grid_title, mat_leg, rel_widths = c(3, .7))
ggsave(here("figs", "ms_figs", "figsupp2_mat_mod_reg.png"), width=11.5, height=4, units="in")
```

Takeaways:

-   MAT is positively correlated with log response ratio for AS activity and SOC%
...
```{r random effects model with map as a moderator variable, echo=FALSE}
# Calculate random-effects model
...
```{r Figure S3 - plot map as a moderator variable, echo=FALSE}
# MAP is significant for alkaline phosphatase and  arylsulfatase
# Make vector of indicators 
map_indicators <- c("alkaline_phosphatase", "arylsulfatase")
...
map_title <- ggdraw() +
  draw_label(expression("Mean annual precipitation"~(mm~yr^-1)), color="black", vjust=0, y=0.05)

map_grid_title <- plot_grid(map_grid, map_title,
                            ncol=1, rel_heights = c(1,0.1))

map_leg <- get_legend(map_aryl)

plot_grid(map_grid_title, map_leg, rel_widths = c(3, .7))
ggsave(here("figs", "ms_figs", "figsupp3_map_mod_reg.png"), width=11.5, height=4, units="in")
```

Takeaways:

-   MAP is negatively correlated with effect sizes for alkaline phosphatase and arylsulfatase activities
...
```{r random effects model with land use as a moderator variable, echo=FALSE}
# Calculate random-effects model
...
```{r Figure S4 - plot land use as a moderator variable, echo=FALSE}
                ...
                .f = ~{
                  es_asp %>% 
                    ...
                    labs(x="Land use", y="Log response ratio",
                         title=glue::glue({filter(indicator_labs_df, indicator==.x)$label})) + 
                    scale_fill_manual(values=c("#476F84FF", "#72874EFF"), name="Management") +
                    theme_katy_grid()
                })
...
lu_grid <- plot_grid(lu_bd + theme(legend.position="none", 
                     ...
                     lu_acd + theme(legend.position="none",  
                                   axis.title.x=element_blank(), axis.text.x=element_text(angle=45, hjust=1)),
...
lu_title <- ggdraw() +
  draw_label("Land use", color="black", vjust=0, y=0.05)

lu_grid_title <- plot_grid(lu_grid, lu_title,
                            ncol=1, rel_heights = c(1,0.1))

lu_leg <- get_legend(lu_alk + theme(legend.box.margin = margin(0, 0, 0, 12)))

plot_grid(lu_grid_title, lu_leg, rel_widths = c(3, .7))
ggsave(here("figs", "ms_figs", "figsupp4_lu_mod_box.png"), width=10.5, height=8, units="in")
```

Takeaways:

-   Land use is a significant moderator variable for bulk density, aggregate stability, and phosphatase activity (both acid and alkaline)

-   Significant variability in all indicators between land uses, doesn't seem to be a clear pattern though

### Tillage as a moderator variable

Calculate random-effects model with tillage as a moderator variable, generate table of results, and generate table of indicators for which tillage is a significant moderator variable:

```{r random effects model with tillage as a moderator variable, echo=FALSE}
es_asp_rma_till <- es_asp %>%
  group_by(indicator) %>%
  nest() %>%
  mutate(rma_obj = purrr::map(data, ~rma(yi, vi, slab = project, mod = ~ factor(till), data=.x, method="REML"))) %>%
  mutate(rma_tidy = purrr::map(rma_obj, broom::tidy))
...
Plot relationship between tillage and indicator variables for which it is a significant predictor:

```{r Figure S5 - plot tillage as a moderator variable, echo=FALSE}
# Tillage is significant for KSSL WSA, acid phosphatase, and alkaline phosphatase
till_indicators <- c("kssl_wsa", "acid_phosphatase", "alkaline_phosphatase")

till_plots <- purrr::map(.x = till_indicators,
                .f = ~{
                  es_asp %>% 
                    filter(indicator == .x) %>%
...
till_kssl <- pluck(till_plots, 1)
till_acd <- pluck(till_plots, 2)
till_alk <- pluck(till_plots, 3)

till_grid <- plot_grid(till_kssl + theme(legend.position="none", 
                                   axis.title.x=element_blank(), axis.text.x=element_text(angle=45, hjust=1)),
                     till_acd + theme(legend.position="none", axis.title.x=element_blank(),
                                     axis.title.y=element_blank(), axis.text.x=element_text(angle=45, hjust=1)), 
...
till_title <- ggdraw() +
  draw_label("Tillage", color="black", vjust=0, y=0.07)

till_grid_title <- plot_grid(till_grid, till_title,
                            ncol=1, rel_heights = c(1,0.1))

till_leg <- get_legend(till_alk + theme(legend.box.margin = margin(0, 0, 0, 12)))

plot_grid(till_grid_title, till_leg, rel_widths = c(3, .7))
ggsave(here("figs", "ms_figs" ,"figsupp5_till_mod_box.png"), width=10.5, height=8, units="in")
```

Takeaways:

-   Tillage is a significant moderator variable for aggregate stability (which makes sense to me) and phosphatase activity (acid and alkaline, which makes less sense to me)

-   Effect size of aggregate stability is smaller for no-till vs. conservation tillage or perennial systems - maybe just reflecting a Ref vs SHM difference?

### Clay content as a moderator variable

Calculate random-effects model with clay content as a moderator variable, generate table of results, and generate table of indicators for which clay content is a significant moderator variable:

```{r random effects model with clay% as a moderator variable, echo=FALSE}
# Calculate random-effects model
...
Clay content is missing from 6 studies, so even though the QM test is significant for bulk density, Yoder MWD, SOC%, and ACE, I don't trust reporting that information.

### Compile all significant moderator variables into one table

```{r Table 5 - combine all significant moderator variables into one table, echo=FALSE}
...
till2 <- till_rma_report_table_sig %>%
  mutate(term = str_replace(term, "factor\\(till\\)", "Till - ")) %>%
  mutate(indicator = str_replace_all(indicator, indicator_labs))
...
write_csv(sig_mod_var_report_table, here("figs", "ms_figs","table5_sig_mod_var_report_table.csv"))
```

# Recommendations for incorporating soil health information into soil survey

```{r derive scores for ranking indicators, include=FALSE}
# scores for each indicator based on management sensitivity - want to give each indicator a score 1-4 based on ranked effect sizes
indicator_es_score <- es_rma_summary %>%
  ungroup() %>%
  group_by(indicator) %>%
  summarize(avg_es = mean(es)) %>%
  mutate(es2 = avg_es^2, # square the average es so negative values are treated equally
         es_rank = order(order(es2, decreasing=TRUE)),
         es_score = ifelse(es_rank >=10, 4,
                        ifelse(es_rank>= 7, 3,
                               ifelse(es_rank>=4, 2, 1))))

# Make a dataframe for scores (scoring explanation and methods are in my table spreadsheet)
indicator_scores <- data.frame(indicator = c("bulk_density", "kssl_wsa", "yoder_agg_stab_mwd", "soc_pct",
                                             "ace", "pox_c", "soil_respiration", "bglucosidase", "bglucosaminidase",
                                             "acid_phosphatase", "alkaline_phosphatase", "arylsulfatase"),
                               mgmt_score = c(4,1,1,2,2,3,3,2,4,4,3,1),
                               mod_score = c(1,1,1,2,1,1,1,1,1,1,2,2),
                               interp_score = c(1,2,2,1,1,3,3,4,4,4,4,4),
                               applic_score = c(1,1,2,1,2,1,4,2,3,3,3,3))
indicator_scores_long <- indicator_scores %>%
...
```{r Figure 6 - rank indicators, echo=FALSE}
# plot heatmap of scores
cat_labs <- c("mgmt_score" = "Management sensitivity",
              "mod_score" = "Broad applicability",
              "interp_score" = "Interpretability",
              "applic_score" = "Feasibility")

cat_plotting_order <- c("mgmt_score", "interp_score", "mod_score", "applic_score")

score_colors <- c("1" = "#238b45",
              "2" = "#66c2a4",
              "3" = "#b2e2e2",
              "4" = "#edf8fb")

score_plot <- ggplot(indicator_scores_long, aes(x=fct_relevel(category, cat_plotting_order), 
                                                y=fct_relevel(indicator, rev(indicator_plotting_order)), 
                                                fill=score)) +
  geom_tile() +
  scale_x_discrete(labels=cat_labs) +
  scale_y_discrete(labels=indicator_labs) +
  labs(x="Category", y="Indicator") +
...
ggsave(here("figs", "ms_figs","fig6_score_plot.png"), height=7, width=5, units="in", dpi=400)
```

# Using SHAPE scores to standardize response to management (kind of like another version of the meta-analysis)
...
# Pull out data
shape_lmer_df <- shape_all %>%
  select(dsp_pedon_id, project, soil, label, score_mean_soc_mapped, score_mean_ac_mapped, 
         score_mean_resp_mapped, score_mean_ace_mapped, score_mean_ksslwas_mapped) %>%
  pivot_longer(score_mean_soc_mapped:score_mean_ksslwas_mapped, names_to="indicator", values_to="shape_score") %>%
  mutate(indicator = case_when(grepl("_soc_", indicator) ~ "soc",
                              grepl("_ac_", indicator) ~ "poxc",
                              grepl("_resp_", indicator) ~ "resp",
                              grepl("_ace_", indicator) ~ "ace",
                              grepl("ksslwas_", indicator) ~ "wsa"))
...
All indicators have significant effect of management on SHAPE score when differences in projects are accounted for. For, SOC, ACE, and WSA, SHAPE scores follow pattern of Ref > SHM > BAU (all significantly different). For POX-C, SHM and Ref are both significantly > BAU, but are not significantly different from each other. For respiration, Ref is > SHM and BAU, but SHM and BAU are not significantly different from each other.

Plot SHAPE scores for each indicator, grouped by management:

```{r Figure 5 - boxplot of SHAPE scores for each indicator grouped by management}
# Make list of indicators for facet_wrap labels
shape_ind_labs <- c("soc" ="SOC %",
                    "poxc" = "POX-C",
                    "resp" = "Respiration",
...
ggplot(shape_lmer_df, aes(x=factor(label, levels=c("BAU", "SHM", "Ref")), 
  ...
  facet_wrap(vars(factor(indicator, levels=c("wsa", "soc", "ace", "poxc", "resp"),
                            labels=c("Aggregate stability", "SOC %", "ACE protein", "POX-C", "Respiration")))) +
  geom_text(data=shape_letters, aes(y=1.1, label=letter)) +
...
ggsave(here("figs", "ms_figs", "shape_scores_all.png"), height=6, width=9, units="in", dpi=400)
```

```{r version for sssa with just aggregate stability and ace}
# Pull out data for plotting
shape_plot_sssa <- shape_plot_df %>%
  filter(indicator=="wsa" | indicator=="ace") %>%
  mutate(label = factor(label, levels=c("BAU", "SHM", "Ref")))

# Boxplot of SHAPE scores by management
ggplot(shape_plot_sssa, aes(x=label, 
                          y=shape_score, 
                          fill=label)) +
  geom_boxplot_pattern(lwd=0.7, pattern= "stripe", 
                       pattern_alpha=0.7, pattern_density = 0.01,
                       aes(pattern_fill = label,
                           pattern_angle = label,
                           pattern_spacing = label,
                           pattern_color = label)) +
  ...
  theme_katy_grid() +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  facet_wrap(vars(indicator), labeller=labeller(indicator=shape_ind_labs)) +
  geom_text(data=shape_letters %>% filter(indicator=="wsa" | indicator=="ace"), 
            aes(y=1.1, label=letter)) +
  labs(y="SHAPE Score (Peer Group Percentile)",
       x="Management") +
  theme_katy() +
  theme(legend.position="none")
ggsave(here("figs", "sssa", "wsa_ace_shape.png"), height=5, width=8, units="in", dpi=400)
```
