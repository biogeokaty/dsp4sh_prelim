...
pedon_clim <- soc_pedon %>%
  left_join(select(site_clim2, dsp_pedon_id, project, soil, mat, map, climate), by=c("dsp_pedon_id", "project", "soil")) %>%
  group_by(soil) %>%
  fill(climate, .direction = "downup")  # fill in missing climate grouping for rows that don't have lat/long
...
surf_all <- horizon_clim %>%
  ...
  summarize(across(c(soc_pct,bulk_density, tn_pct:yoder_agg_stab_mwd, p_h:ace), mean)) %>%
  left_join(select(site_clim2, dsp_pedon_id, mat, map, climate), by="dsp_pedon_id") %>%
