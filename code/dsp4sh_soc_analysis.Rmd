...
soc_horizon_filt <- soc_horizon_all %>%
  filter(!soil %in% soils_exclude100)
...
First, need to do some data processing, including fixing incorrect latitude and longitude.
```{r fix pedon location data}
...
Then, pull in MAT/MAP data from WorldClim.
```{r worldclim data}
...
I also want to calculate the average MAT and MAP for each project, so I can make an alternative version of the SOC stock boxplot for Pedometrics where sites are arranged by MAP.
```{r average mat/map for each site}
